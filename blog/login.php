...
require_once "modelo/RepositorioUsuarios.php";
    ...
    if(isset($_POST["correo"])){
        ...
        $rep=new RepositorioUsuarios($conexion->getConexion());
        
        if($usuario=$rep->existeUsuario($correo, $password)){
            $_SESSION["usuario"]=serialize($usuario);
            header("Status: 301 Moved Permanently");
            header("Location: index.php");
            exit;
        }else{
            $mensaje="<p class='alert alert-danger'>Usuario o contraseña incorrectos</p>";
        }
    }
?>
